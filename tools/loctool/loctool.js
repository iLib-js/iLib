...
const validCommands = [ 
	null: {
		definitions: [
		    { name: 'version', alias: rb.getStringJS('v'), type: Boolean, description: rb.getStringJS('Print the version number.') }
        ],
        usage: [
            {
            	header: 'loctool',
            	content: rb.getStringJS('A tool to extract strings and localize various projects.')
            },
            {
            	header: rb.getStringJS('synopsis'),
            	content: rb.getStringJS('$ node loctool.js <options> <command>')
            },
            {
            	header: rb.getStringJS('Command List'),
            	content: [
            	    { name: rb.getStringJS('help'), summary: rb.getStringJS('Display this help information.') },
            	    { name: rb.getStringJS('localize'), summary: rb.getStringJS('Extract strings from source code and write localized resource files appropriate to the type of project.') },
            	    { name: rb.getStringJS('export'), summary: rb.getStringJS('Export the strings from the database into a file that can be passed to the translators.') },
            	    { name: rb.getStringJS('import'), summary: rb.getStringJS('Import the translation results from the translators back into the database.') }
            	]
            }
        ]
	}, 
	'localize': {
	    definitions: [
	        { name: 'topic', type: String, description: rb.getStringJS('the topic to display help on'), defaultOption: true }
	    ],
	    usage: [
		    {
		        header: rb.getStringJS('loctool help'),
		        content: rb.getStringJS('Get help about a loctool command')
		    },
		    {
		        header: rb.getStringJS('synopsis'),
		        content: rb.getStringJS('$ node loctool.js help <options>')
		    }
	    ]		
	}, 
	'export': {
...
const { command, argv } = clc(validCommands);
