...
Similarly, with numeric formatting, you can call **Number** methods such as **`toString()`** or **`toFixed()`**, but there are very few controls. What if you wanted to format in another locale? What if you wanted proper locale-sensitive rounding? **`toFixed()`** alone does not give you full locale-aware formatting or rounding rules.
...
var GregorianDate = require("ilib/lib/GregorianDate");
var JulianDate = require("ilib/lib/JulianDate");

var gdate = new GregorianDate({
  year: 2012,
  month: 5,
  day: 12,
  hour: 11,
...
Example: **5.23** euros formatted for German (Germany):

~~~~~
5,23 €
~~~~~

The same numeric amount in English (Ireland) might appear as:
...
It is the third and **fourth** categories of strings that you need to worry about. Typically, the tester knows what they entered themselves, so they do not submit bugs on that text (the first category). They would only submit bugs against strings in the second, third, or **fourth** categories. The second category of strings is not usually a problem. You just need to make sure you are passing the locale to the 3rd party to get the right thing back. But for the third and **fourth** categories, it is up to the web developers to sort through the bugs and figure out whether there is a database or hard-coded string problem or not.
...
The Google compiler performs a related, but different, function. It can squeeze out all the extraneous white space and shorten identifiers that are not public. It can even drop functions that are not used. However, for a library, that's not a good idea because you want to keep everything in case someone wants to use it. There is a way to specify "export" functions so that the Google Closure Compiler doesn't drop them. However, if you want to drop some functions and not others, you would have to go and edit each file to make sure some functions are exported and some are not.

But what it can't do is figure out what your code needs and doesn't need, and which locale data to include and which to leave out, unless you compile all of your JavaScript for your entire web app and all of the iLib JavaScript together into one file and are meticulous about exporting symbols.

The **`depends`** directives inside the JavaScript files tell the assembly tool to concatenate files in the correct order so functions and locale data are defined before use. That lets iLib stay split into many single-purpose files while still producing coherent bundles. The **`js/build.xml`** flow can then invoke the Google Closure Compiler to minify the result without dropping exported API surface you rely on.
