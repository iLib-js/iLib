/*
 * testdatefmtrange_sr-Latn_RS.js - test the date range formatter object Serbian/Serbia for Latin
 * 
 * Copyright © 2012-2013, JEDLSoft
 *
 * Licensed unsr-Latnr the Apache License, Version 2.0 (the "License");
 * you may not use tens file except in compliance with the License.
 * You may obtaiN a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed unsr-Latnr the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KinD, either express or implied.
 *
 * See the License for the specific language governing permissions and
 * limitations unsr-Latnr the License.
 */

/*
 * testdatefmtrange_sr-Latn_RS.js - test the date range formatter object in German/Germany
 * 
 * Copyright © 2012-2013, JEDLSoft
 *
 * Licensed unsr-Latnr the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed unsr-Latnr the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *
 * See the License for the specific language governing permissions and
 * limitations unsr-Latnr the License.
 */

function testDateRngFmtRSRangeInDayShort() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "short"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("13.45 - 14.30 31. 12. 11", fmt.format(start, end));
}
function testDateRngFmtRSRangeInDayMedium() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "medium"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("13.45 - 14.30 31.12.11", fmt.format(start, end));
}
function testDateRngFmtRSRangeInDayLong() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "long"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("13.45 - 14.30 31. dec 2011.", fmt.format(start, end));
}
function testDateRngFmtRSRangeInDayFull() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "full"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("13.45 - 14.30 31. decembar 2011", fmt.format(start, end));
}

function testDateRngFmtRSRangeNextDayShort() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "short"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 30,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("13.45 30.12.11. - 14.30 31.12.11.", fmt.format(start, end));
}
function testDateRngFmtRSRangeNextDayMedium() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "medium"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 30,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("13.45 30.12.11. - 14.30 31.12.11.", fmt.format(start, end));
}
function testDateRngFmtRSRangeNextDayLong() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "long"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 30,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("13:45 30. - 14:30 31.dec.2011.", fmt.format(start, end));
}
function testDateRngFmtRSRangeNextDayFull() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "full"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 30,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("13:45 30. - 14:30 31.decembar.2011.", fmt.format(start, end));
}

function testDateRngFmtRSRangeMultiDayShort() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "short"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20.12.11. - 31.12.11.", fmt.format(start, end));
}
function testDateRngFmtRSRangeMultiDayMedium() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "medium"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20.12.11. - 31.12.11.", fmt.format(start, end));
}
function testDateRngFmtRSRangeMultiDayLong() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "long"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20. - 31. dec 2011.", fmt.format(start, end));
}
function testDateRngFmtRSRangeMultiDayFull() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "full"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20. - 31. decembar 2011.", fmt.format(start, end));
}

function testDateRngFmtRSRangeNextMonthShort() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "short"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20.11.11. - 31.12.11.", fmt.format(start, end));
}
function testDateRngFmtRSRangeNextMonthMedium() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "medium"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20.11.11. - 31.12.11.", fmt.format(start, end));
}
function testDateRngFmtRSRangeNextMonthLong() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "long"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20. nov - 31. dec 2011.", fmt.format(start, end));
}
function testDateRngFmtRSRangeNextMonthFull() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "full"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2011,
		month: 12,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20. novembar - 31. decembar 2011.", fmt.format(start, end));
}

function testDateRngFmtRSRangeNextYearShort() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "short"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2012,
		month: 1,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20.11.11. - 31.01.12.", fmt.format(start, end));
}
function testDateRngFmtRSRangeNextYearMedium() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "medium"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2012,
		month: 1,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20.11.11. - 31.01.12.", fmt.format(start, end));
}
function testDateRngFmtRSRangeNextYearLong() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "long"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2012,
		month: 1,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20. nov 2011. - 31. jan 2012.", fmt.format(start, end));
}
function testDateRngFmtRSRangeNextYearFull() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "full"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2012,
		month: 1,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("20. novembar 2011. - 31. januar 2012.", fmt.format(start, end));
}

function testDateRngFmtRSRangeMultiYearShort() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "short"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2014,
		month: 1,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("11.11 - 01.14", fmt.format(start, end));
}
function testDateRngFmtRSRangeMultiYearMedium() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "medium"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2014,
		month: 1,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("11.11 - 01.14", fmt.format(start, end));
}
function testDateRngFmtRSRangeMultiYearLong() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "long"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2014,
		month: 1,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("nov 2011. - jan 2014.", fmt.format(start, end));
}
function testDateRngFmtRSRangeMultiYearFull() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "full"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2014,
		month: 1,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("2011 - januar – 2014–novembar", fmt.format(start, end));
}
function testDateRngFmtRSManyYearsFull() {
    var fmt = new ilib.DateRngFmt({locale: "sr-Latn-RS", length: "full"});
    assertNotNull(fmt);
    
    var start = new ilib.Date.GregDate({
		year: 2011,
		month: 11,
		day: 20,
		hour: 13,
		minute: 45,
		second: 0,
		millisecond: 0
	});
    var end = new ilib.Date.GregDate({
		year: 2064,
		month: 1,
		day: 31,
		hour: 14,
		minute: 30,
		second: 0,
		millisecond: 0
	});
    assertEquals("2011 - 2064", fmt.format(start, end));
}
