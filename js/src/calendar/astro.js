...
function jhms(j) {
	...
	return new Array(Math.floor(ij / 3600), Math.floor((ij / 60) % 60), Math
			.floor(ij % 60));
}
...
var oterms = [ -4680.93, -1.55, 1999.25, -51.38, -249.67, -39.05, 7.12, 27.87,
		5.79, 2.45 ];
...
var nutArgMult = [ 0, 0, 0, 0, 1, -2, 0, 0, 2, 2, 0, 0, 0, 2, 2, 0, 0,
...
function ecliptoeq(jd, Lambda, Beta) {
	...
	Ra = rtd(Math.atan2((Math.cos(eps) * Math.sin(dtr(Lambda)) - (Math
			.tan(dtr(Beta)) * Math.sin(eps))), Math.cos(dtr(Lambda))));
	log += "RA = " + Ra + "\n";
	Ra = fixangle(rtd(Math.atan2((Math.cos(eps) * Math.sin(dtr(Lambda)) - (Math
			.tan(dtr(Beta)) * Math.sin(eps))), Math.cos(dtr(Lambda)))));
	Dec = rtd(Math.asin((Math.sin(eps) * Math.sin(dtr(Lambda)) * Math
			.cos(dtr(Beta)))
...
var deltaTtab = [ 121, 112, 103, 95, 88, 82, 77, 72, 68, 63, 60, 56,
...
var EquinoxpTerms = [ 485, 324.96, 1934.136, 203, 337.23, 32964.467,
...
function equinox(year, which) {
	...
	for (i = j = 0; i < 24; i++) {
		S += EquinoxpTerms[j]
				* dcos(EquinoxpTerms[j + 1] + (EquinoxpTerms[j + 2] * T));
		j += 3;
	}
